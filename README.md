...
| [codex-worker](plugins/team-skills/skills/codex-worker/README.md) | kostya | internal, codex, helper | Internal helper для запуска Codex-воркера. Вызывается из /codereview-dual и /sprint-codex. Запуск через движок плагина codex@openai-codex (app-server, без bubblewrap); fallback — legacy codex exec. |
