...
| [dashboards](plugins/team-skills/skills/dashboards/README.md) | kostya | dashboards, observability, ui, data-sources, review | Как строить внутренние дашборды, которым можно верить. Не про то, как написать вьюху: про провенанс чисел, пустые и протухшие состояния, обе темы, доступ до первого рендера и владельца-человека. Плюс каталог источников данных контура — что откуда читается и что откуда НЕ достаётся. Каждый принцип оплачен инцидентом: молчаливо протухшие снапшоты стоили четырёх суток веры в старые числа, а витрина, открытая «просто дали ссылку», простояла публичной год. |
| [env-audit](plugins/team-skills/skills/env-audit/README.md) | Костя | audit, onboarding, agents, secrets, resources, process, architecture | Приёмка рабочего места: коллектор фактов + бриф до 20 КБ + справочники по темам. Одиннадцать регламентов, секреты и 1Password, сеть, расход токенов и связка с Codex, архитектура проектов, знакомство с человеком. Клинап применяется только после показанного диффа и «да», запись HANDOFF и CHANGELOG проверяется по факту в песочнице; в финале — отчёт и инструкция человеку |
...
| [spec-brainstorm](plugins/team-skills/skills/spec-brainstorm/README.md) | kostya | spec, brainstorm, workflow, interactive | Интерактивное со-создание новой спецификации проекта в режиме «вопрос-ответ». Проводит через структурированную разведку (проблема → цель → фазы → архитектура → DoD), затем делегирует финальную запись файла в /spec-writer. Принципиально интерактивный — задаёт вопросы по одному через AskUserQuestion, не пишет файл пока человек не финализировал. |
