...
| Шаг | На Windows |
|---|---|
| 1. Факты | секреты — **программой** (см. ниже), остальное вручную по таблицам |
...
**Секреты под Windows ищет коллектор, а не ты.** Решение владельца от 22.09.2026 (пересмотр D13):

```
py -3 collect.py --only secrets --output %USERPROFILE%\audit-<дата>\facts-secrets.json
```

Работает `--only secrets` и `--scan-file`; любой другой набор разделов — код 5. Что меняется
на Windows: права файлов не читаются (`mode` = `null`, рядом `acl: "not_checked"` — ACL
проверяешь руками по таблице ниже), история PowerShell сканируется как история shell,
из обхода дома исключены `AppData\Local\Temp`, `Packages`, `Microsoft` и кэши браузеров,
а `AppData` идёт в числе приоритетных каталогов. Нет `git` — корни помечаются `no_git`,
нет `ssh-keygen` — парольная фраза ключа `not_checked`.

Самописные сканеры секретов больше не нужны: именно в них была самая дорогая ошибка
первого прогона — два значения вебхуков ушли в вывод команды.
